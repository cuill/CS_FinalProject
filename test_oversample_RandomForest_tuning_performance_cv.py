...
classifier = RandomForestClassifier(n_estimators=200,max_depth=16,min_samples_leaf=5,max_features='sqrt',min_samples_split=2,random_state=0,criterion='entropy')
...
for n in kfolds:
    ...
    accuracy = cross_val_score(classifier,X_test,y_test,cv=n,scoring='accuracy')
    precisions = cross_val_score(classifier,X_test,y_test,cv=n,scoring='precision')
    recalls = cross_val_score(classifier,X_test,y_test,cv=n,scoring='recall')
    roc_auc = cross_val_score(classifier,X_test,y_test,cv=n,scoring='roc_auc')
